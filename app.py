...
# ════════════════════════════════════════════════════════════
    ...
    # ────────────────────────────────────────────────────────
    # Normativa Verifactu (RD 1007/2023): una factura emitida NUNCA
    # puede pasar a "anulada". Solo se permite marcar como pagada
    # o vencida. Las correcciones se hacen con factura rectificativa.
    # ────────────────────────────────────────────────────────
    TRANSICIONES = {
        "pendiente": ["pagada", "vencida"],
        "pagada": [],        # ← antes permitía "anulada", AHORA NO
        "vencida": [],
        "anulada": [],       # ← ya no se puede alcanzar, pero por datos antiguos
        "rectificada": [],
    }
            ...
            with col_btn_rect1:
                if st.button("💾 Guardar rectificativa", key="btn_guardar_rect", use_container_width=True):
                    if num:
                        ...
                        if exito:
                            try:
                                # NO usamos "anulada". Marcamos como "rectificada" para
                                # mantener la trazabilidad sin romper inalterabilidad.
            ...
            if selected_row is not None and 0 <= selected_row < len(inv_display):
                ...
                if not factura_seleccionada.empty:
                    ...
                    if opciones_cambio:
                        ...
                        st.info("🔒 No se permiten cambios de estado para esta factura (normativa Verifactu).")
                    ...
                    with col4:
                        ...
                        # RD 1007/2023: una factura emitida NO se puede modificar ni borrar.
                        # Las correcciones se hacen mediante factura rectificativa vinculada.
                        # ────────────────────────────────────────────────────────
...
# ════════════════════════════════════════════════════════════
    ...
    if invoices.empty and expenses.empty:
        ...
        if data_inv.empty and data_exp.empty:
            ...
            kpi_kwargs = {"year": year_seleccionado}
            if mes_seleccionado != "Todos los meses":
                kpi_kwargs["month"] = mes_seleccionado
            ...
            with st.container(border=True):
                ...
                k1.metric("🟢 Ingresos Totales", money(s_kpi["total_ventas"]))
                k2.metric("🔴 Gastos Totales", money(s_kpi["total_gastos"]))
                k3.metric("📄 Facturas Emitidas", f"{s_kpi['num_invoices']} uds.")
                k4.metric("📊 Ticket Promedio", money(s_kpi["ticket_promedio"]))

                st.markdown("---")

                st.caption("🏛️ **RENDIMIENTO Y LIQUIDACIÓN FISCAL (BASES & IVA)**")
                k5, k6, k7, k8 = st.columns(4)
                k5.metric("📈 Beneficio Bruto (Base)", money(s_kpi["beneficio_bruto"]))
                k6.metric("📄 IVA Repercutido", money(s_kpi["iva_repercutido"]))
                k7.metric("🧾 IVA Soportado", money(s_kpi["iva_soportado"]))
                k8.metric(
                    "💶 IVA Neto",
                    money(s_kpi["iva_neto"]),
                    delta="A ingresar" if s_kpi["iva_neto"] > 0 else "A compensar",
                    delta_color="inverse" if s_kpi["iva_neto"] > 0 else "normal"
                )
